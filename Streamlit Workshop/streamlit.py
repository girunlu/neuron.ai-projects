   ...
   with col1:
    ...
    if uploaded_file is not None:
        ...
        if st.button("Apply"):
            ...
            if color != "Gray-scale":
                new_image = np.concatenate((new_image, np.ones((new_image.shape[0], new_image.shape[1], 1), dtype=int) * int(values * 2.55)), axis=-1)
...
ignore = """with tab2: #failed chatbot page because of regulations by streamlit
    if False:
